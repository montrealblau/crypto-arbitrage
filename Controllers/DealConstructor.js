...
exports.get = (exchange, paris, callback) => {
    ...
    bot.getAllPairsTradingNow(exchange, function(res) {
        availablepairs = res;
        mylog('gettingAllPairsTradingNow...');
        ...
        bot.getTickers(exchange, function(res) {
            ...
            bot.orderBook(exchange, function(res) {
                ...
                paris.forEach(function(e) {
                    ...
                    deals = deals.concat(arbitrage.run(e, dummRes));
    ...
    function getPairsForDeal(deal) {
        ...
        if (deal.length === 4 && deal[0] != undefined) {
            ...
            if (availablepairs.includes(pair1)) {
                tmp = [];
                tmp.push('BUY');
                tmp.push(pair1)
                ...
                result.push(tmp);
            } else if (availablepairs.includes(pairReverse1)) {
                tmp = [];
                tmp.push('SELL');
            ...
            if (availablepairs.includes(pair2)) {
                tmp = [];
                tmp.push('BUY');
                tmp.push(pair2);
                ...
                result.push(tmp);
            } else if (availablepairs.includes(pairReverse2)) {
                tmp = [];
                tmp.push('SELL');
            ...
            if (availablepairs.includes(pair3)) {
                ...
                result.push(tmp);
            } else if (availablepairs.includes(pairReverse3)) {
                tmp = [];
                tmp.push('SELL');
